...
| PCA / TSA / NA | `colony`(1) | 형태 일반, 색상 무관 | ✅ 기존 방식(변경 없음) |
| VRBA | `typical` / `possible` / `atypical` / `non_target`(4) | 색상+halo 강도, **지름 0.5mm 미만 하드 제외**(px 임계값으로 근사, §4.3) | 🔧 예전 `vrbaCandidateClass()` 이식 |
...
const TYPE_CLASS_SCHEMES = {
  ...
  NA:   { classes: ['colony'],                                      basis: 'morphology', auto: true },
...
### 4.3 VRBA 4단계 + 하드 사이즈 컷오프

예전 `고체배지/colony-counter-v2.html`의 다음 로직을 이식한다(신규 agar 코드에 맞게 함수명 조정):

- `rgb2hsv`, `rgb2lab` 변환
- `hueGateScore(h, gate=[280,20])` — 자주→빨강→분홍 범위 소프트 스코어
- `ringMeanColor()` 기반 `haloScore` — 콜로니 주변 링의 배경 대비 밝기/채도 차이
...
- **이 컷오프는 VRBA 전용 예외임을 코드 주석에 명시한다** — `CLAUDE.md`의 "단일 조건 하드 임계값 금지" 원칙에 대한 의도적·국소적 예외이며, 다른 타입/`validateInstance` 소프트 스코어링 로직에는 전혀 영향을 주지 않는다. 이 예외를 다른 타입까지 확대하지 않는다.
